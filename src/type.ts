...
export class Position {
  ...
  add(position: Position) {
    ...
    if (typeof this.m_x === "string" && this.m_x.startsWith("~")) {
      x1 =
        Number.parseFloat(this.m_x.substring(1)) +
        <number>global.player.Position.X;
    } else if (typeof this.m_x === "number") {
      x1 = this.m_x;
    }
    if (typeof this.m_y === "string" && this.m_y.startsWith("~")) {
      y1 =
        Number.parseFloat(this.m_y.substring(1)) +
        <number>global.player.Position.Y;
    } else if (typeof this.m_y === "number") {
      y1 = this.m_y;
    }
    if (typeof this.m_z === "string" && this.m_z.startsWith("~")) {
      z1 =
        Number.parseFloat(this.m_z.substring(1)) +
        <number>global.player.Position.Z;
    } else if (typeof this.m_z === "number") {
      z1 = this.m_z;
    }
    if (typeof position.X === "string" && position.X.startsWith("~")) {
      x2 =
        Number.parseFloat(position.X.substring(1)) +
        <number>global.player.Position.X;
    ...
    if (typeof position.Y === "string" && position.Y.startsWith("~")) {
      y2 =
        Number.parseFloat(position.Y.substring(1)) +
        <number>global.player.Position.Y;
    ...
    if (typeof position.Z === "string" && position.Z.startsWith("~")) {
      z2 =
        Number.parseFloat(position.Z.substring(1)) +
        <number>global.player.Position.Z;
...
export class SlotTypeGenerator {
  static enderChest = (slotNumber: number): CustomSlotType =>
    `slot.enderchest.${slotNumber}`;
  static hotBar = (slotNumber: number): CustomSlotType =>
    `slot.hotbar.${slotNumber}`;
  static inventory = (slotNumber: number): CustomSlotType =>
    `slot.inventory.${slotNumber}`;
  static horse = (slotNumber: number): CustomSlotType =>
    `slot.horse.${slotNumber}`;
  static villager = (slotNumber: number): CustomSlotType =>
    `slot.villager.${slotNumber}`;
}
...
export const ENTITY_MAP: { [key: number]: string } = {
  64: "item",
  69: "xp_orb",
  65: "tnt",
  66: "falling_block",
